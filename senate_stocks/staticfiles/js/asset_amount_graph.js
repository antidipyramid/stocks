...
var assetAmountMargin = {top: 30, right: 30, bottom: 120, left: 50},
  assetAmountWidth = 600 - assetAmountMargin.left - assetAmountMargin.right,
  assetAmountHeight = 400 - assetAmountMargin.top - assetAmountMargin.bottom;

// append the svg object to the body of the page
var assetAmountSvg = d3.select("#asset-amount-graph")
  .append("svg")
  .attr("width", assetAmountWidth + assetAmountMargin.left + assetAmountMargin.right)
  .attr("height", assetAmountHeight + assetAmountMargin.top + assetAmountMargin.bottom)
  .append("g")
  .attr("transform",
    "translate(" + assetAmountMargin.left + "," + assetAmountMargin.top + ")");

var assetAmountTooltip = d3.select("#asset-amount-graph")
    .append("div")
    .attr("class", "tooltip")
...
function loadAssetAmountGraph(data) {
  ...
  // X axis
  var assetAmountX = d3.scaleBand()
    .range([ 0, assetAmountWidth ])
    .domain(data.keys())
    .padding(0.5);
  assetAmountSvg.append("g")
    .attr("transform", "translate(0," + assetAmountHeight + ")")
    .call(d3.axisBottom(assetAmountX)
      .tickSize(0)
      .tickPadding(15))
    .selectAll("text")
  ...
  // Add Y axis
  var assetAmountY = d3.scaleLinear()
    .domain([-(0.02 * maxY), maxY])
    .range([ assetAmountHeight, -(0.02 * maxY)]);
  assetAmountSvg.append("g")
    .call(d3.axisLeft(assetAmountY)
      .tickSize(0)
      .tickValues([]));
  ...
  // Bars
  assetAmountSvg.selectAll("mybar")
    .data(data.entries())
    .enter()
    .append("rect")
    .attr("rx",BAR_ROUND)
  ...
  // Text labels
  assetAmountSvg.selectAll("labels")
    .data(data.entries())
    .enter()
    .append("text")
