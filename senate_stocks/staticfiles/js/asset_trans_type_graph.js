...
var assetTypeSvg = d3.select("#asset-type-graph")
  .append("svg")
    .attr("width", assetTypeWidth + assetTypeMargin.left + assetTypeMargin.right)
    .attr("height", assetTypeHeight + assetTypeMargin.top + assetTypeMargin.bottom)
  .append("g")
    .attr("transform",
          "translate(" + assetTypeMargin.left + "," + assetTypeMargin.top + ")");

var assetTypeTooltip = d3.select("#asset-type-graph")
    .append("div")
    .attr("class", "tooltip")
...
function loadAssetTransTypeGraph(data) {
  ...
  // Add Y axis
  var assetTypeY = d3.scaleLinear()
    .domain([-(0.02 * maxY), maxY])
    .range([ assetTypeHeight, -(0.02 * maxY)]);
  assetTypeSvg.append("g")
    .call(d3.axisLeft(assetTypeY)
      .tickSize(0)
      .tickValues([]));
