...
var active_margin = { top: 0, right: 50, bottom: 0, left: 200 },
  activeWidth = 400 - active_margin.left - active_margin.right,
  activeHeight = 200 - active_margin.top - active_margin.bottom;

// append the activeSvg object to the body of the page
var activeSvg = d3
  .select("#most-active-senators")
  .append("svg")
  // .call(responsivefy)
...
function loadActiveSenatorGraph(data) {
  ...
  let sortedData = Array.from(data.entries())
    .sort((a, b) => d3.descending(a[1], b[1]))
    .slice(0, 9);
  console.log(sortedData);
  console.log(count);

  d3.select("#most-active-senators")
    .select("svg")
    .attr(
      "height",
      sortedData.length * BAR_HEIGHT * 2 +
        active_margin.top +
        active_margin.bottom
    );
  console.log(
    sortedData.length * BAR_HEIGHT + active_margin.top + active_margin.bottom
  );
  ...
  var activeX = d3
    ...
    .range([0, activeWidth]);
  var activeXAxis = activeSvg
    .append("g")
    .attr("transform", "translate(0," + activeHeight + ")")
    .call(d3.axisBottom(activeX).tickSize(0).tickValues([]))
    .select(".domain")
    .remove();
  ...
  activeSvg
    .append("g")
    .call(d3.axisLeft(activeY).tickPadding(5).tickSize(0))
  ...
  activeSvg
    ...
    .on("mousemove", function (e, d) {
      ...
      activeTooltip.style("left", x + "px").style("top", y + "px");
    })
    .on("mouseout", function (d) {
      d3.select(this).style("opacity", "0.7");
      // .style("fill", BAR_COLOR);

      activeTooltip.style("visibility", "hidden").style("display", "none");
    });
