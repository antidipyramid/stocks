...
function loadOwnerGraph(data) {
  ...
  owner_svg
    ...
    .call(d3.axisBottom(owner_x).tickSize(0).tickPadding(15))
    .selectAll("text")
  ...
  owner_svg.append("g").call(d3.axisLeft(owner_y).tickSize(0).tickValues([]));
  ...
  owner_svg
    ...
    .on("mousemove", function (e, d) {
      ...
      ownerTooltip.style("left", x + "px").style("top", y + "px");
    })
    .on("mouseout", function (d) {
      d3.select(this).style("opacity", "0.7");
      // .style("fill", BAR_COLOR);

      ownerTooltip.style("visibility", "hidden").style("display", "none");
    });
