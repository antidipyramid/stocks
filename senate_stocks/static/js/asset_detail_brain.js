...
const apiUrl = '/api/assets/' + window.location.pathname.split('/').pop();
...
async function processAssetData(assetJson) {
  for (const trade of assetJson.asset_related_trades) {
    ...
    assetTradeAmountMap.set(
      trade.amount,
      assetTradeAmountMap.get(trade.amount) + 1
    );
    ...
    if (
      ...
      assetTransTypeMap.set(
        trade.transaction_type,
        assetTransTypeMap.get(trade.transaction_type) + 1
      );
    } else {
